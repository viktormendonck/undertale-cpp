...
void Loox::SpawnBullet(ResourceManager* pResourceManager)
{
	...
	m_Bullets.clear();
	int randAttack = 0; //utils::RandInRange(0, 1);
	if (randAttack == 0)
	{
		for (int i{}; i <= 6; ++i) {
			int randLocation{ utils::RandInRange(0,11) };
	...
	else
	{
		for(int i{}; i<=1; ++i)
		{
			int randLocation{ };//utils::RandInRange(4,11) };
			int RandomizeDirection{ utils::RandInRange(1,2) };
			m_Bullets.push_back(new LooxAttack2(spawnLocations[randLocation], 10, pResourceManager->m_BulletAnimatedSprites[2], m_Collider, RandomizeDirection == 1, i * 0.5, 5));
		}
	}
}
